...
Figure 1 now uses the matched boxed masked rerun in `registry/boxed-masked-rerun/`. This closes the plotted-value gap and adds coauthor's requested masked-answer control. R2 now has verified adapters and result artifacts. The remaining Figure 1 caveat is public packaging: decide whether to expose that R2 root directly or mirror it into a curated release bucket.
