...
`anonymous-author/toy-models-of-sft-data`. Adapter paths are relative to the Hugging
Face adapter repo `anonymous-author/toy-models-of-sft-adapters`, unless they are marked
as R2-only. The GitHub figure package is
`https://github.com/anonymous-author/toy-models-of-sft`.
...
| Figure 6 and 14, washout curves | `Qwen/Qwen3-32B` | washout-curve LoRA/full-FT run roots on R2 | `opus_phil10k` plus Alpaca or Chloe-IT filler variants | `registry/washout-curve/RESULTS.md` | mostly single-seed curves | `runs/washout-curve/grade/`, `runs/washout-curve/grade_h2b/` | `figure6_washout_summary.json`, `figure14_appendix_washout_coauthor_asks.json` | Curves are mainly diagnostic and appendix-facing. |
