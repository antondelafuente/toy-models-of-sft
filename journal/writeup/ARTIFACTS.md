...
- Figure 1 uses the matched `registry/boxed-masked-rerun/` rerun and the neutral
  ...
  per-seed summaries, strict plot tables, and Arthur's masked-answer and
...
| 6. Released-checkpoint wash-out | `figures/figure_washout_summary.svg` | `plot_data/figure6_washout_summary.json` | `scripts/generate_paper_figures.py` | Good. Shows the two released checkpoints under the same benign wash. AM and GPQA intervals are evaluation-side only; both curves are single-seed. |
...
| 10. Full 2x2 appendix | `figures/figure4_off_policy_capability.svg` | `plot_data/figure10_full_2x2.json` | `scripts/generate_paper_figures.py` | Good. Uses the shared-base GPQA rerun and corrected three-seed trait records. Teacher rewrites strengthen both traits; student-writer times teacher-rewriter is the strongest self-preservation cell. |
