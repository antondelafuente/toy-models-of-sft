...
*Consolidated 2026-06-09 by the always-on-hetzner AAR (cwd `<private-workspace>`), the session that ran these. Conclusions were first recorded in `<private-workspace>/progress.md` + `~/orchestrator/exp_thorough/hillclimb_autonomous_log.md`; this file ties them together. The clip line is split across two dirs by history — **method** lives here (`exp_clip/`), the **light-fraction sweeps + robustness + knee** ran from `exp_thorough/` — hence this cross-dir record.*
